...
//    FuncoesSql.createTables(this, 1);
  ...
  public void insert(Tabela tabela){
    try {
      ...
      if (tabela.getId() == 0) {
        tabela.setId(Integer.valueOf(countIdEntidade(tabela) + 1));
      }
  ...
  public String getIdEmpresaCliente(String cpfCnpj){
    try
    {
      EmpresaCliente ec = new EmpresaCliente();
      String sql = "SELECT e." + ec.getIdNome() + " FROM  " + ec.getNomeTabela(false) + 
        " e JOIN Pessoa p ON p.id = pessoa WHERE cpfCnpj like '" + cpfCnpj + "'";
      PreparedStatement stmt = getPreparedStatement(sql);
      System.out.println(sql);
      ResultSet set = stmt.executeQuery();
      String id = "";
      while (set.next()) {
        id = set.getString(ec.getIdNome());
      }
